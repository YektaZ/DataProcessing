...
for i = 1:length(block_trials)
    ...
    [Xvel_avg, Yvel_avg, Xvel_SEM, Yvel_SEM, Avg_Time, Speed_avg, Speed_SEM] = AVGES_5(trials_successful',Trial,fs);
    ...
    if numpt>1
        ...
        plot(Avg_Time, Speed_avg + Speed_SEM,'Color',[0.7,0.8,0.8])
        plot(Avg_Time, Speed_avg - Speed_SEM,'Color',[0.7,0.8,0.8])
...
for sub = 1:num_subs
    ...
    for i = 1:num_blocks
        ...
        BlockAvgSpeed{i,sub} = S.AVG_Vel.Speed{i};
        BlockAvgSEM{i,sub} = S.AVG_Vel.Speed_SEM{i};
    end
end
...
for block_cnt = 1:num_blocks
    ...
    for j = 1:num_subs
        ...
        if length(idx)>1
            ...
            tempSp(1:l) = tempSp(1:l) + sqrt(BlockXvelAvg{block_cnt,j}(idx).^2 + BlockYvelAvg{block_cnt,j}(idx).^2);
    ...
    for j = 1:num_subs
        ...
        if l>1
            ...
            tempSp_SS(1:l) = tempSp_SS(1:l) + (BlockAvgSpeed{block_cnt,j}(idx) - Speed_avg{block_cnt}(1:l)).^2;
        end
    end
    ...
    plot(Avg_Time(id), Speed_avg{block_cnt}(id) + Speed_SEM{block_cnt}(id),'LineWidth',1.5,'Color',[0.7,0.8,0.8])
    plot(Avg_Time(id), Speed_avg{block_cnt}(id) - Speed_SEM{block_cnt}(id),'LineWidth',1.5,'Color',[0.7,0.8,0.8])
...
n1 = sqrt(length(nonzeros(Depths_EFB(:))));
n2 = sqrt(length(nonzeros(Depths_LFB(:))));

AVG = [mean(nonzeros(Depths_EFB(:))), mean(nonzeros(Depths_LFB(:)))];
SEM = [std(nonzeros(Depths_EFB(:)))/n1, std(nonzeros(Depths_LFB(:)))/n2];
...
%% load the data for each subject and count the peaks in speed profile
